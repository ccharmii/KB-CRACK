# # /KB-CRACK/non_financial_analysis/prompt.py
# 비재무 신용위험 평가를 위한 시스템 및 사용자 프롬프트 구성
...
def user_prompt(ind, quarter):
    """
    지표 정보와 평가 분기를 포함한 사용자 프롬프트 생성
    입력: ind(지표 딕셔너리), quarter(평가 분기 문자열)
    출력: 모델 입력용 사용자 프롬프트 문자열
    """
